...
public class World {
		...
		try {
			...
			for (int xx = 0; xx < map.getWidth(); xx++) {
				for (int yy = 0; yy < map.getHeight(); yy++) {
					...
					} else if (pixelAtual == 0XFFC6BCBA) {
						// _
						NPC__ npc_ = new NPC__(xx * 16, yy * 16, 9, 15, Entity.NPC_EN);
						Game.entities.add(npc_);
	...
	public static boolean isFreeDynamic(int xnext, int ynext, int width, int height) {// para checar se a proxima
																						// posisao esta livre
