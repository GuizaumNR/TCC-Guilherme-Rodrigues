...
public class Player extends Entity {
	...
	public void tick() {
		...
		moved = false;
		if (right && World.isFreeP((int) (x + speed), this.getY())) {
			moved = true;
			dir = right_dir;
			x += speed;

		} else if (left && World.isFreeP((int) (x - speed), this.getY())) {
			moved = true;
			dir = left_dir;
			x -= speed;

		}
		if (up && World.isFreeP(this.getX(), (int) (y - speed))) {
			moved = true;
			dir = up_dir;
			y -= speed;

		} else if (down && World.isFreeP(this.getX(), (int) (y + speed))) {
			moved = true;
			dir = down_dir;
			y += speed;
		...
		if (!isDamaged) {
			if (dir == right_dir) {
				...
				if (hasGun) {
					...
					g.drawImage(Entity.RIGHTGUN_EN, this.getX() - Camera.x + 9, this.getY() - Camera.y + 6 - z, null);
